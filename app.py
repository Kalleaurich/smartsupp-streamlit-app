...
RAW_SUPPLEMENT_DATA_PATH = "02_U-Phase/fitness_supplements_dataset.csv"
RANDOM_STATE = 42
...
with tab1:
    ...
    try:
        ...
        # --- Konsistenz mit BCAA-Sonderregel oben: wenn BCAA in den Top-3 die
        # spezielle Geld-Spar-Warnung erhält, soll BCAA aus denselben Gründen auch
        # hier im allgemeinen Geld-Spar-Warnsystem auftauchen ---
        bcaa_in_top3 = top3[top3["Supplement"] == "BCAA"]
        if not bcaa_in_top3.empty and "BCAA" not in money_waste["Supplement"].values:
            money_waste = pd.concat([money_waste, bcaa_in_top3], ignore_index=True)
    ...
    if submitted_reg:
        try:
            ...
            with st.container(border=True):
                ...
                col_a.metric(
                    "Erwartetes Gewicht laut Modell:",
                    f"{predicted_weight:.1f} kg",
                    help=(
                        "Kein Wert für die Zukunft, sondern ein Vergleichswert für genau jetzt: "
                        "Er zeigt, welches Gewicht historisch zu deinem eingegebenen Verhalten "
                        "(Kalorien, Schritte, Training) gepasst hat. Weicht dein tatsächliches "
                        "Gewicht stark davon ab, deutet das auf ein Plateau oder einen "
                        "unerwarteten Fortschritt hin."
                    ),
                )
                col_b.metric(
                ...
                st.caption(
                    "ℹ️ Das **erwartete Gewicht** ist kein Wert für die Zukunft, sondern ein "
                    "Vergleichswert für genau jetzt: Es zeigt, welches Gewicht historisch zu "
                    "deinem eingegebenen Verhalten (Kalorien, Schritte, Training) gepasst hat. "
                    "Weicht dein tatsächliches Gewicht stark davon ab, kann das auf ein Plateau "
                    "oder einen unerwarteten Fortschritt hindeuten."
                )
